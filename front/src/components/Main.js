...
export default function Main() {
    ...
    useEffect(() => {
        ...
        const fetchAndShowMarkers = async () => {
            const NORMAL_SCALE = 0.75;  // 기본 크기
            const HOVER_SCALE  = 0.92;  // 호버 시 살짝 확대 (0.88~1.00 사이로 취향 조절)
            ...
            try {
                ...
                const overlay = new window.kakao.maps.CustomOverlay({ zIndex: 10000, yAnchor: 1.02 });
                ...
                const markers = mergedParkingList
                    .map((park) => {
                        ...
                        // 기본/호버 이미지
                        const normalImage = buildMarkerImage(park, 0.75);
                        const hoverImage  = buildMarkerImage(park, 0.92);
                        ...
                        // 모바일 터치 대응(선택)
                        window.kakao.maps.event.addListener(marker, "touchstart", () => marker.setImage(hoverImage));
                        window.kakao.maps.event.addListener(marker, "touchend",   () => marker.setImage(normalImage));
